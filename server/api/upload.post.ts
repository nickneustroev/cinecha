...
import { getImportMovieCount, resolveImportMaxMoviesLimit } from '../utils/tmdb'
...
function createMovieLimitError(locale: string, maxMovies: number) {
  const message = locale.toLowerCase().startsWith('ru')
    ? `Извините, максимум фильмов в архиве на обработку - ${maxMovies}`
    : `Sorry, the maximum number of movies allowed in the archive for processing is ${maxMovies}`
...
export default defineEventHandler(async (event): Promise<EnrichedImportData> => {
  ...
  const maxMovies = resolveImportMaxMoviesLimit()
  if (maxMovies !== null) {
    const movieCount = getImportMovieCount({
    ...
    if (movieCount > maxMovies) {
      throw createMovieLimitError(locale, maxMovies)
    }
  }
